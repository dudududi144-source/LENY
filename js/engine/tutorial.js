...
export function tutorialStart(li){
 ...
 if(li===0&&!S.tutorial){RT.tut=1;
  later(()=>TTS.say(STR.tutorial.welcome),700);}}

export function tutorialUpdate(p){
 if(RT.tut===1){
  RT.tutDist=(RT.tutDist||0)+Math.abs(p.vx);
  RT.curHint=STR.tutorial.walkHint;RT.hintTimer=200;
  if(RT.tutDist>130){RT.tut=2;AU.sfx('power');
   TTS.say(STR.tutorial.jumpPraise);}}
 else if(RT.tut===2){
  RT.curHint=STR.tutorial.jumpHint;RT.hintTimer=200;
  if(p.vy<-2){RT.tut=3;AU.sfx('power');
   TTS.say(STR.tutorial.gatePraise);}}
 else if(RT.tut===3){
  RT.curHint=STR.tutorial.gateHint;RT.hintTimer=200;}}
